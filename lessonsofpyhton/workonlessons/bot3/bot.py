...
# Завантаження збереженого стану фільмів з файлу
def load_films():
    try:
        with open('films.txt', 'r') as file:
            films_str = file.read()
            films = eval(films_str)
    except FileNotFoundError:
        films = {}
    return films

# Збереження стану фільмів у файл


def save_films(films):
    with open('films.txt', 'w') as file:
        films_str = str(films)
        file.write(films_str)
...
async def set_photo(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        film_site_url = data['film_site_url']
        film_description = data['film_description']
        film_rating = data['film_rating']
        films = load_films()  # Завантаження стану фільмів
        films[film_name] = {'site_url': film_site_url, 'description': film_description,
                            'rating': film_rating, 'photo': film_photo}
        save_films(films)  # Збереження оновленого стану фільмів
        await state.finish()
        await message.answer(text='Супер! Новий фільм успішно додано до бібліотеки.')
